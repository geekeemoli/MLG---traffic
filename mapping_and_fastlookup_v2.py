...
def map_detectors_to_road_graph(
    ...
    for city in cities:
        ...
        G_roads = get_density(G_roads) #v prihodnje CSV path dodaj!
        graphs[city] = G_roads
...
## --------------------------------------------------------------------
##  STEP 2: CREATE A FAST LOOKUP MAP
## --------------------------------------------------------------------

def create_detector_lookup(city_graphs: Dict[str, nx.Graph]) -> Dict[str, tuple]:
    """
    Creates a fast lookup map from detid -> (node_id, city_name).
    node_id is the (u, v, k) tuple representing the road segment. 
    Because multiple detid-s can be located on the same node_id. That is a fast look-up
    """
    detector_map = {}
    
    for city_name, graph in city_graphs.items():
        # Iterate through only the nodes that have detectors
        for node_id, data in graph.nodes(data=True):
            if data.get('has_detector', False):
                # The 'detectors' attribute is a list of detid strings
                for detid in data['detectors']:
                    detector_map[detid] = (node_id, city_name)
                    
    print(f"Lookup map built with {len(detector_map)} total detector entries.")
    return detector_map
